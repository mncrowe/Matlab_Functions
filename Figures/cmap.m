...
%    vector: 2 element vector of [Min Max] (default: values of current figure if one exists)
...
arguments
    options.R       (:,:) double          = set_R_default()
...
function R = set_R_default()

    % if there's an open window, set R to the colormap limits, else use R = [-1 1]:
